...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        if (!isFeatureChecked && !isPageChecked) {
            ...
            // Default fallback based on language/engine combination
            if (this.isJavaSelenium(lang, eng)) {
                promptKeys.push('SELENIUM_JAVA_PAGE_ONLY');
            } else if (this.isPythonPlaywright(lang, eng)) {
                promptKeys.push('PLAYWRIGHT_PYTHON_PAGE_ONLY');
            }
            return promptKeys;
        }
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                promptKeys.push('PYTEST_WITH_PLAYWRIGHT_STEPS');
            } else {
            ...
            // Feature file only
            if (this.isPythonPlaywright(lang, eng)) {
                promptKeys.push('PYTEST_PLAYWRIGHT_ONLY');
            } else {
                promptKeys.push('CUCUMBER_ONLY');
            }
     ...
     */
    isPythonPlaywright(language, engine) {
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Supported combinations: Java/Selenium, Python/Playwright.`;
